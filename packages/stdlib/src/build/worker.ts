...
/**
 * Generate worker.ts entry point for sources scheduling
 *
 * NOTE: This is a sources-only build for scheduled data sync.
 * For full blocks/pages support, use the runtime build system
 * at packages/runtime/src/build/index.ts instead.
 */
export function generateWorkerEntry(config: HandsJson): string {
  ...
  const imports: string[] = [
    "// Auto-generated by `hands build` - sources only",
    "// For full blocks/pages support, use the runtime build system",
    "",
  ...
  // Worker export - sources-only mode
  code.push(
    "// Worker export (sources-only)",
    "export default {",
    "  async fetch(request: Request, env: Env) {",
    '    return new Response("Sources worker - use runtime build for blocks/pages", { status: 200 })',
    "  },",
    "",
  )
