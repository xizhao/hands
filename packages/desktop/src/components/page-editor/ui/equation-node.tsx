...
import { ArrowElbowDownLeft, MathOperations } from '@phosphor-icons/react';
          ...
          >
            {element.texExpression.length > 0 ? (
              ...
              <div className="flex h-7 w-full items-center gap-2 whitespace-nowrap text-muted-foreground text-sm">
                <MathOperations className="size-6 text-muted-foreground/80" />
      ...
      <Popover modal={false} onOpenChange={setOpen} open={open}>
        <PopoverTrigger asChild>
          ...
          >
            ...
            {element.texExpression.length === 0 && (
              <span>
                <MathOperations className="mr-1 inline-block h-[19px] w-4 py-[1.5px] align-text-bottom" />
                New equation
              </span>
            )}
          </div>
  ...
  return (
    ...
    >
      ...
      <Button className="px-3" onClick={onClose} variant="brand">
        Done <ArrowElbowDownLeft className="size-3.5" />
      </Button>
    </PopoverContent>
  );
}
