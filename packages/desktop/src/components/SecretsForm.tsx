  ...
  ({
    ...
    return (
      <div className="space-y-1">
        ...
        <p className="text-[10px] text-muted-foreground/60 -mt-0.5 mb-1">{spec.description}</p>
...
export const SecretsForm = memo(({ output, onSaved }: SecretsFormProps) => {
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      const stillMissing = missingSecrets.filter((s) => s.required && !secretsToSave[s.key]);
      ...
      <div className="px-3 py-2 border-b border-blue-500/20 bg-blue-500/10">
        <div className="flex items-center gap-2">
          ...
          <div className="flex-1 min-w-0">
            <h3 className="text-xs font-medium text-blue-300">Secrets Required</h3>
            <p className="text-[10px] text-blue-400/70 truncate">{output.message}</p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="p-3 space-y-3">
        ...
        {existingSecrets.length > 0 && missingSecrets.length > 0 && (
          <div className="pt-2 border-t border-border/30">
            <p className="text-[10px] text-muted-foreground/50 mb-1">Already configured:</p>
        ...
        <Button
          ...
          disabled={saving}
          size="xs"
          className={cn("w-full h-7 text-xs gap-1.5", "bg-blue-600 hover:bg-blue-700 text-white")}
        >
          {saving ? (
            <>
