...
export function CaptureActionPanel() {
  ...
  const [_panelId, setPanelId] = useState<string>("");
  ...
  const selectedWorkbookName =
    workbooks.find((w) => w.id === selectedWorkbook)?.name || "Select workbook...";

  return (
    <div className="h-screen w-screen flex flex-col bg-transparent" onClick={handleClose}>
      <div ref={contentRef} className="flex flex-col gap-2" onClick={(e) => e.stopPropagation()}>
        ...
        {screenshotPath && imgWidth > 0 && imgHeight > 0 && (
          <div className="p-5" onMouseDown={handleDragStart}>
            <div className="relative" style={{ width: imgWidth, height: imgHeight }}>
              ...
              <div className="absolute inset-0 rounded-md bg-gradient-to-r from-blue-500/50 via-purple-500/50 to-blue-500/50 blur-sm animate-glow-pulse" />
        ...
        {!isAnalyzing && (
          <div
            className="px-5 pb-3 flex flex-wrap items-center gap-2"
