...
import { useBackgroundTask, type BackgroundTaskResult } from "@/hooks/useBackgroundTask"
...
export function useSaveSpec() {
  ...
  return useMutation({
    mutationFn: async ({ sourceId, spec }: { sourceId: string; spec: string }): Promise<{ success: boolean }> => {
      if (!port) throw new Error("Runtime not available")
...
export function useSaveSecrets() {
  ...
  return useMutation({
    mutationFn: async (secrets: Record<string, string>): Promise<{ success: boolean; saved: string[] }> => {
      if (!port) throw new Error("Runtime not available")
...
export function useStreamingSync() {
  ...
  const sync = useCallback(async (sourceId: string) => {
    ...
    try {
      const eventSource = new EventSource(`http://localhost:${port}/sources/${sourceId}/sync/stream`)
