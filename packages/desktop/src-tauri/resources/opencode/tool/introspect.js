// @bun
// ../plugin/src/tools/introspect.ts
import { tool } from "@opencode-ai/plugin";
var introspectTool = tool({
...
Use this to understand what a hands app does before making changes.`,
  args: {
    path: tool.schema.string().optional().describe("Path to hands.config.ts. Defaults to current directory."),
    detailed: tool.schema.boolean().optional().describe("Include handler source code analysis")
  },
  async execute(args, ctx) {
    ...
    const result = ["# Hands App Structure", ""];
    ...
    if (monitorsMatch) {
      ...
      for (const [, name, config] of monitorDefs) {
        ...
        result.push(`- **${name}**`);
        if (scheduleMatch)
          result.push(`  - Schedule: ${scheduleMatch[1]}`);
        if (handlerMatch)
          result.push(`  - Handler: ${handlerMatch[1]}`);
      }
      result.push("");
    }
