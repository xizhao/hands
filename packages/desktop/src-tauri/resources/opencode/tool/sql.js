// @bun
// ../plugin/src/tools/sql.ts
import { tool } from "@opencode-ai/plugin";
import postgres from "postgres";
var DATABASE_URL = process.env.HANDS_DATABASE_URL || "postgres://hands:hands@localhost:5433/hands_db";
var sql = null;
function getClient() {
  if (!sql) {
...
function formatTable(rows) {
  ...
  const dataRows = rows.map((r) => headers.map((h, i) => String(r[h] ?? "").padEnd(widths[i])).join(" | "));
...
- Describe table: SELECT column_name, data_type FROM information_schema.columns WHERE table_name = 'your_table'`,
  ...
  async execute(args, ctx) {
    ...
    const isDestructive = lowerQuery.startsWith("drop") || lowerQuery.startsWith("truncate") || lowerQuery.startsWith("delete") && !lowerQuery.includes("where");
