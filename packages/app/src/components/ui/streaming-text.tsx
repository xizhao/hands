...
const TypingCursor = memo(({ className }: { className?: string }) => (
  <motion.span
    className={cn("inline-block w-[2px] h-[1.1em] bg-primary ml-0.5 align-middle", className)}
    animate={{ opacity: [1, 0] }}
    transition={{
      duration: 0.5,
      repeat: Infinity,
...
export const StreamingText = memo(
  ({ text, isStreaming = false, className, cursorClassName }: StreamingTextProps) => {
    return (
...
export const AnimatedTextReveal = memo(
  ({ text, className, delay = 0, stagger = 0.02 }: AnimatedTextRevealProps) => {
...
export const GradientStreamingText = memo(
  ({ text, isStreaming = false, className }: GradientStreamingTextProps) => {
    if (!isStreaming) {
