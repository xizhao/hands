...
import { generateTypesForFiles, buildSchemaMap, type SchemaMap } from "./type-generator.js"
 ...
 */
export function createPgTypedRunner(
  workbookDir: string,
  db: PGlite
): PgTypedRunner {
  ...
  async function runFile(filePath: string): Promise<void> {
    ...
    for (const [typesPath, content] of typesMap) {
      ...
      console.log(`[pgtyped] Generated ${relative(workbookDir, typesPath)} (${parsed.queries.length} queries)`)
    }
  }
...
export { extractQueriesFromSource, type ExtractedQuery, type ParsedFile } from "./parser.js"
export { generateTypesForFiles, generateTypesFile } from "./type-generator.js"
