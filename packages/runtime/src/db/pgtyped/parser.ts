...
import { Project, SyntaxKind, type SourceFile, type TaggedTemplateExpression } from "ts-morph"
import { parseTSQuery, type TSQueryAST, type Param } from "@pgtyped/parser"
...
export function extractQueriesFromSource(source: string, filePath: string): ParsedFile {
  ...
  for (const tagged of taggedTemplates) {
    ...
    try {
      ...
      const parseErrors = events.filter((e) => e.type === "error")
      if (parseErrors.length > 0) {
        for (const err of parseErrors) {
          errors.push(`Query "${queryName}": ${err.message}`)
        }
      }
