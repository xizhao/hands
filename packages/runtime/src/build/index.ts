...
export async function build(
  ...
  try {
    ...
    // Check for legacy workbook (has src/index.tsx but no blocks)
    const legacySrcPath = join(workbookDir, "src/index.tsx")
    const isLegacyWorkbook = existsSync(legacySrcPath) && blocksResult.blocks.length === 0

    if (isLegacyWorkbook && options.verbose) {
      console.log("Detected legacy workbook (src/index.tsx), using legacy build mode")
    }
    ...
    // Use legacy mode if workbook has src/index.tsx and no blocks
    const workerTs = isLegacyWorkbook
      ? generateLegacyWorkerEntry(config)
      : generateWorkerEntry(
          workbookDir,
          config,
          pagesResult.pages,
...
/**
 * Generate legacy worker entry point for workbooks with src/index.tsx
 * This is for backwards compatibility with old-style workbooks
 */
function generateLegacyWorkerEntry(config: HandsConfig): string {
  return `// Generated by hands build system - LEGACY MODE
// This workbook uses the old src/index.tsx pattern
// Consider migrating to blocks/ architecture with: hands migrate

import app from "../src/index";

// Re-export the app as default
export default app;
`
}
