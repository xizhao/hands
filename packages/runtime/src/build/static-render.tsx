...
function PlateNode({ node }: { node: TDescendant }): React.ReactElement | null {
  ...
  switch (element.type) {
    ...
    case "tableHeader":
      return (
        <th className="px-4 py-2 border border-gray-200 bg-gray-50 font-semibold">{children}</th>
      );
    ...
    case "callout": {
      ...
      return (
        <div
          className={`rounded-lg border p-4 my-4 ${variantClasses[variant] || variantClasses.info}`}
        >
          {children}
        </div>
      );
