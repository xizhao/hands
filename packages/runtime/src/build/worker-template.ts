...
export function generateWorkerTemplate(opts: WorkerTemplateConfig): string {
  ...
  // Generate block metadata registry (for parentDir, title, etc.)
  const blockMetaRegistry = blocks.map((block) => {
    const filename = block.id.includes("/") ? block.id.split("/").pop() : block.id
    return `  "${block.id}": { parentDir: "${block.parentDir}", title: "${filename}" },`
  }).join("\n")
...
// Block metadata registry (parentDir, title)
const BLOCK_META: Record<string, { parentDir: string; title: string }> = {
${blockMetaRegistry}
};
...
async function buildManifest(workbookDir: string, workbookId: string, db: any) {
  ...
  // Get blocks from registry with metadata
  // BLOCK_META is generated at build time with parentDir info
