 ...
 */
function _compareAstStructure(a: any, b: any): boolean {
  if (a.tagName !== b.tagName) return false;
  if (a.children.length !== b.children.length) return false;

  for (let i = 0; i < a.children.length; i++) {
    if (!_compareAstStructure(a.children[i], b.children[i])) return false;
  }
 ...
 */
function _getStructureSummary(node: any, depth = 0): string {
  ...
  for (const child of node.children || []) {
    result += _getStructureSummary(child, depth + 1);
  }
