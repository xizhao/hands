...
function SkeletonElement({
  ...
  return (
    <div data-skeleton-id={nodeId} className="rounded-md bg-muted/20 animate-pulse" style={style} />
  );
}
...
export function generateSkeletonFromAST(node: EditableNode | null): React.ReactNode {
  ...
  if (node.tagName === "#fragment") {
    return <>{node.children.map((child) => generateSkeletonFromAST(child))}</>;
  }
...
export function estimateSkeletonHeight(node: EditableNode | null): number {
  ...
  const childrenHeight = node.children.reduce(
    (sum, child) => sum + estimateSkeletonHeight(child),
    0,
  );
