...
import { Plate, PlateContainer, PlateContent, usePlateEditor, createPlatePlugin } from 'platejs/react'
...
export function PlateVisualEditor({
  source,
  onSourceChange,
  className,
}: PlateVisualEditorProps) {
  ...
  const SourceSyncPlugin = useMemo(() => createPlatePlugin({
    key: 'source-sync',
    extendEditor: ({ editor }) => {
      const { apply } = editor
