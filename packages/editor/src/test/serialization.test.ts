 ...
 *
 * Tests roundtrip serialization of MDX content through the editor.
 * Verifies that markdown/MDX can be deserialized and serialized correctly.
 */
...
describe('MDX Roundtrip Serialization', () => {
  describe('Basic Markdown', () => {
    it('roundtrips paragraph text', () => {
      ...
      const parsed = editor.api.markdown.deserialize(mdx);
...
3. Third item`;
  ...
  <BarChart xKey="status" yKey="count" />
    ...
    it('roundtrips MDX component with expression props', () => {
      ...
      // Note: query prop might not be preserved depending on stdlib rules
    });
