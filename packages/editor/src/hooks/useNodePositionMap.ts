 ...
 */
export function findNodeAtPoint(container: HTMLElement, x: number, y: number): string | null {
  const containerRect = container.getBoundingClientRect();
 ...
 */
export function getAncestorNodeIds(container: HTMLElement, nodeId: string): string[] {
 ...
 */
export function getDescendantNodeIds(container: HTMLElement, nodeId: string): string[] {
  const element = container.querySelector(`[data-node-id="${nodeId}"]`);
  if (!element) return [];
