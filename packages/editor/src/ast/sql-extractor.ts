...
import { astVisitor, parse as parseSql } from "pgsql-ast-parser";
...
function extractTemplateLiteralSql(
  quasi: OxcNode,
  source: string,
): { sql: string; rawSource: string } {
...
function walkAst(
  node: OxcNode | null,
  visitor: (node: OxcNode, parent: OxcNode | null) => void,
  parent: OxcNode | null = null,
): void {
...
function _findAssignmentTarget(_node: OxcNode, parent: OxcNode | null): string | undefined {
...
export function extractDataDependencies(source: string): DataDependencies {
      ...
      if (node.type === "VariableDeclarator" && node.id?.type === "Identifier") {
        ...
        if (queryLoc) {
          ...
          const query = queries.find(
            (q) => q.loc.start === queryLoc?.start && q.loc.end === queryLoc?.end,
          );
          if (query) {
  ...
  for (const binding of bindings) {
    for (const jsxEl of binding.jsxElements) {
      ...
      if (
        !jsxDataUsages.some(
          (u) => u.elementLoc.start === jsxEl.start && u.elementLoc.end === jsxEl.end,
        )
      ) {
        jsxDataUsages.push({
          elementLoc: jsxEl,
...
export function getDataDependencySummary(deps: DataDependencies): string {
  ...
  const writeCount = deps.queries.filter((q) =>
    ["insert", "update", "delete"].includes(q.type),
  ).length;
