...
import type { IteratorContext, JsxNode, NodePath, RenderedNode, RenderedScene } from "../types";
...
function matchChildToAst(
  ...
  const matchByTag = astChildren.find((n) => n.type === "element" && n.tagName === tagName);
 ...
 */
export function findNodeByPath(scene: RenderedScene, path: NodePath): RenderedNode | null {
 ...
 */
export function findNodeAtPoint(scene: RenderedScene, x: number, y: number): RenderedNode | null {
